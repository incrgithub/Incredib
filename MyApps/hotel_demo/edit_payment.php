...
session_start();
 	...
 	while ($row = mysql_fetch_array($result))
	{
		...
		$old_pay_amount= $ecol['cr_amount'];
	 	$old_extra_note= $ecol['extra_note'];
		
		 $query_i="select * from customer_master where cust_id='$cust_id' ";
	  $rs = mysql_query($query_i);   while ($row_data = mysql_fetch_array($rs)){$i_data[] = $row_data;}  foreach($i_data as $ecol) {$old_due=$ecol['cust_balance']; }
  		}
	
	$new_balance=$old_due + $old_pay_amount;
	
	$update="update customer_master set cust_balance='$new_balance' where cust_id='$cust_id'";
  ...
  $update="update customer_master set cust_balance='$balance' where cust_id='$cust_id'";
  @mysql_query($update) or die(" Query for Customer Balance Master Table has problem  "); 


	$update="update all_tran set tran_date='$pdate',cr_amount='$due_amount',extra_note='$extra' where tran_id='$pay_id'";
