...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// User_Level_ID
		$this->User_Level_ID = new cField('userlevels', 'userlevels', 'x_User_Level_ID', 'User_Level_ID', '`User_Level_ID`', '`User_Level_ID`', 3, -1, FALSE, '`User_Level_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->User_Level_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['User_Level_ID'] = &$this->User_Level_ID;

		// User_Level_Name
		$this->User_Level_Name = new cField('userlevels', 'userlevels', 'x_User_Level_Name', 'User_Level_Name', '`User_Level_Name`', '`User_Level_Name`', 200, -1, FALSE, '`User_Level_Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["User_Level_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["User_Level_ID"]);
			elseif (isset($_GET["User_Level_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["User_Level_ID"]);
			else
