...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Timezone
		$this->Timezone = new cField('timezone', 'timezone', 'x_Timezone', 'Timezone', '`Timezone`', '`Timezone`', 200, -1, FALSE, '`Timezone`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Timezone'] = &$this->Timezone;

		// Default
		$this->_Default = new cField('timezone', 'timezone', 'x__Default', 'Default', '`Default`', '`Default`', 202, -1, FALSE, '`Default`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('Timezone', $rs))
				ew_AddFilter($where, ew_QuotedName('Timezone', $this->DBID) . '=' . ew_QuotedValue($rs['Timezone'], $this->Timezone->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Timezone@", ew_AdjustSql($this->Timezone->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("timezoneview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("timezoneview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "timezoneadd.php?" . $this->UrlParm($parm);
		else
			$url = "timezoneadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("timezoneedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("timezoneadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Timezone:" . ew_VarToJson($this->Timezone->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Timezone"]))
				$arKeys[] = ew_StripSlashes($_POST["Timezone"]);
			elseif (isset($_GET["Timezone"]))
				$arKeys[] = ew_StripSlashes($_GET["Timezone"]);
			else
