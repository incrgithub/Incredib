...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Year
		$this->Year = new cField('stats_date', 'stats_date', 'x_Year', 'Year', '`Year`', '`Year`', 2, -1, FALSE, '`Year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Year'] = &$this->Year;

		// Month
		$this->Month = new cField('stats_date', 'stats_date', 'x_Month', 'Month', '`Month`', '`Month`', 16, -1, FALSE, '`Month`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Month->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Month'] = &$this->Month;

		// Date
		$this->Date = new cField('stats_date', 'stats_date', 'x_Date', 'Date', '`Date`', '`Date`', 16, -1, FALSE, '`Date`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Date->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Date'] = &$this->Date;

		// Hits
		$this->Hits = new cField('stats_date', 'stats_date', 'x_Hits', 'Hits', '`Hits`', '`Hits`', 20, -1, FALSE, '`Hits`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("stats_dateview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("stats_dateview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "stats_dateadd.php?" . $this->UrlParm($parm);
		else
			$url = "stats_dateadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("stats_dateedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("stats_dateadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
