...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Year
		$this->Year = new cField('stats_year', 'stats_year', 'x_Year', 'Year', '`Year`', '`Year`', 2, -1, FALSE, '`Year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Year'] = &$this->Year;

		// Hits
		$this->Hits = new cField('stats_year', 'stats_year', 'x_Hits', 'Hits', '`Hits`', '`Hits`', 20, -1, FALSE, '`Hits`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("stats_yearview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("stats_yearview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "stats_yearadd.php?" . $this->UrlParm($parm);
		else
			$url = "stats_yearadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("stats_yearedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("stats_yearadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Year"]))
				$arKeys[] = ew_StripSlashes($_POST["Year"]);
			elseif (isset($_GET["Year"]))
				$arKeys[] = ew_StripSlashes($_GET["Year"]);
			else
