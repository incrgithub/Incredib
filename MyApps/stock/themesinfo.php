...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Theme_ID
		$this->Theme_ID = new cField('themes', 'themes', 'x_Theme_ID', 'Theme_ID', '`Theme_ID`', '`Theme_ID`', 200, -1, FALSE, '`Theme_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Theme_ID'] = &$this->Theme_ID;

		// Theme_Name
		$this->Theme_Name = new cField('themes', 'themes', 'x_Theme_Name', 'Theme_Name', '`Theme_Name`', '`Theme_Name`', 200, -1, FALSE, '`Theme_Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Theme_Name'] = &$this->Theme_Name;

		// Default
		$this->_Default = new cField('themes', 'themes', 'x__Default', 'Default', '`Default`', '`Default`', 202, -1, FALSE, '`Default`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'CHECKBOX');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('Theme_ID', $rs))
				ew_AddFilter($where, ew_QuotedName('Theme_ID', $this->DBID) . '=' . ew_QuotedValue($rs['Theme_ID'], $this->Theme_ID->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Theme_ID@", ew_AdjustSql($this->Theme_ID->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("themesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("themesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "themesadd.php?" . $this->UrlParm($parm);
		else
			$url = "themesadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("themesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("themesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Theme_ID:" . ew_VarToJson($this->Theme_ID->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Theme_ID"]))
				$arKeys[] = ew_StripSlashes($_POST["Theme_ID"]);
			elseif (isset($_GET["Theme_ID"]))
				$arKeys[] = ew_StripSlashes($_GET["Theme_ID"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->Theme_Name->EditValue = $this->Theme_Name->CurrentValue;
