...
class cstats_counterlog extends cTable {
	...
	function __construct() {
		...
		// Update Table
		$this->UpdateTable = "`stats_counterlog`";
		$this->DBID = 'DB';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// IP_Address
		$this->IP_Address = new cField('stats_counterlog', 'stats_counterlog', 'x_IP_Address', 'IP_Address', '`IP_Address`', '`IP_Address`', 200, -1, FALSE, '`IP_Address`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['IP_Address'] = &$this->IP_Address;

		// Hostname
		$this->Hostname = new cField('stats_counterlog', 'stats_counterlog', 'x_Hostname', 'Hostname', '`Hostname`', '`Hostname`', 200, -1, FALSE, '`Hostname`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Hostname'] = &$this->Hostname;

		// First_Visit
		$this->First_Visit = new cField('stats_counterlog', 'stats_counterlog', 'x_First_Visit', 'First_Visit', '`First_Visit`', 'DATE_FORMAT(`First_Visit`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`First_Visit`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->First_Visit->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['First_Visit'] = &$this->First_Visit;

		// Last_Visit
		$this->Last_Visit = new cField('stats_counterlog', 'stats_counterlog', 'x_Last_Visit', 'Last_Visit', '`Last_Visit`', 'DATE_FORMAT(`Last_Visit`, \'%Y/%m/%d %H:%i:%s\')', 135, 9, FALSE, '`Last_Visit`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Last_Visit->FldDefaultErrMsg = str_replace("%s", "/", $Language->Phrase("IncorrectDateYMD"));
		$this->fields['Last_Visit'] = &$this->Last_Visit;

		// Counter
		$this->Counter = new cField('stats_counterlog', 'stats_counterlog', 'x_Counter', 'Counter', '`Counter`', '`Counter`', 3, -1, FALSE, '`Counter`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('IP_Address', $rs))
				ew_AddFilter($where, ew_QuotedName('IP_Address', $this->DBID) . '=' . ew_QuotedValue($rs['IP_Address'], $this->IP_Address->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@IP_Address@", ew_AdjustSql($this->IP_Address->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("stats_counterlogview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("stats_counterlogview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "stats_counterlogadd.php?" . $this->UrlParm($parm);
		else
			$url = "stats_counterlogadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("stats_counterlogedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("stats_counterlogadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "IP_Address:" . ew_VarToJson($this->IP_Address->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["IP_Address"]))
				$arKeys[] = ew_StripSlashes($_POST["IP_Address"]);
			elseif (isset($_GET["IP_Address"]))
				$arKeys[] = ew_StripSlashes($_GET["IP_Address"]);
			else
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->First_Visit->EditValue = ew_FormatDateTime($this->First_Visit->CurrentValue, 9);
		...
		$this->Last_Visit->EditValue = ew_FormatDateTime($this->Last_Visit->CurrentValue, 9);
