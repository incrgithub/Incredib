...
class ca_stock_categories extends cTable {
	...
	function __construct() {
		...
		// Update Table
		$this->UpdateTable = "`a_stock_categories`";
		$this->DBID = 'DB';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Category_ID
		$this->Category_ID = new cField('a_stock_categories', 'a_stock_categories', 'x_Category_ID', 'Category_ID', '`Category_ID`', '`Category_ID`', 3, -1, FALSE, '`Category_ID`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->Category_ID->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Category_ID'] = &$this->Category_ID;

		// Category_Name
		$this->Category_Name = new cField('a_stock_categories', 'a_stock_categories', 'x_Category_Name', 'Category_Name', '`Category_Name`', '`Category_Name`', 200, -1, FALSE, '`Category_Name`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("a_stock_categoriesview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("a_stock_categoriesview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "a_stock_categoriesadd.php?" . $this->UrlParm($parm);
		else
			$url = "a_stock_categoriesadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("a_stock_categoriesedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("a_stock_categoriesadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
