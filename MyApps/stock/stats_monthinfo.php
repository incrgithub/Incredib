...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Year
		$this->Year = new cField('stats_month', 'stats_month', 'x_Year', 'Year', '`Year`', '`Year`', 2, -1, FALSE, '`Year`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Year->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Year'] = &$this->Year;

		// Month
		$this->Month = new cField('stats_month', 'stats_month', 'x_Month', 'Month', '`Month`', '`Month`', 16, -1, FALSE, '`Month`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Month->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Month'] = &$this->Month;

		// Hits
		$this->Hits = new cField('stats_month', 'stats_month', 'x_Hits', 'Hits', '`Hits`', '`Hits`', 20, -1, FALSE, '`Hits`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("stats_monthview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("stats_monthview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "stats_monthadd.php?" . $this->UrlParm($parm);
		else
			$url = "stats_monthadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("stats_monthedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("stats_monthadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
