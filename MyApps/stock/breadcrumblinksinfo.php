...
class cbreadcrumblinks extends cTable {
	...
	function __construct() {
		...
		// Update Table
		$this->UpdateTable = "`breadcrumblinks`";
		$this->DBID = 'DB';
...
// End of mofidication Flexibility of Export Records Options, by Masino Sinaga, May 14, 2012
		...
		// Page_Title
		$this->Page_Title = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Page_Title', 'Page_Title', '`Page_Title`', '`Page_Title`', 200, -1, FALSE, '`Page_Title`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Page_Title'] = &$this->Page_Title;

		// Page_URL
		$this->Page_URL = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Page_URL', 'Page_URL', '`Page_URL`', '`Page_URL`', 200, -1, FALSE, '`Page_URL`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->fields['Page_URL'] = &$this->Page_URL;

		// Lft
		$this->Lft = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Lft', 'Lft', '`Lft`', '`Lft`', 3, -1, FALSE, '`Lft`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Lft->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['Lft'] = &$this->Lft;

		// Rgt
		$this->Rgt = new cField('breadcrumblinks', 'breadcrumblinks', 'x_Rgt', 'Rgt', '`Rgt`', '`Rgt`', 3, -1, FALSE, '`Rgt`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function DeleteSQL(&$rs, $where = "", $curfilter = TRUE) {
		...
		if ($rs) {
			if (array_key_exists('Page_Title', $rs))
				ew_AddFilter($where, ew_QuotedName('Page_Title', $this->DBID) . '=' . ew_QuotedValue($rs['Page_Title'], $this->Page_Title->FldDataType, $this->DBID));
		}
		$filter = ($curfilter) ? $this->CurrentFilter : "";
		ew_AddFilter($filter, $where);
		if ($filter <> "")
	...
	function KeyFilter() {
		...
		$sKeyFilter = str_replace("@Page_Title@", ew_AdjustSql($this->Page_Title->CurrentValue, $this->DBID), $sKeyFilter); // Replace key value
		return $sKeyFilter;
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("breadcrumblinksview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("breadcrumblinksview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "breadcrumblinksadd.php?" . $this->UrlParm($parm);
		else
			$url = "breadcrumblinksadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("breadcrumblinksedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("breadcrumblinksadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function KeyToJson() {
		$json = "";
		$json .= "Page_Title:" . ew_VarToJson($this->Page_Title->CurrentValue, "string", "'");
		return "{" . $json . "}";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			if ($isPost && isset($_POST["Page_Title"]))
				$arKeys[] = ew_StripSlashes($_POST["Page_Title"]);
			elseif (isset($_GET["Page_Title"]))
				$arKeys[] = ew_StripSlashes($_GET["Page_Title"]);
			else
