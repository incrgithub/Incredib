 					 ...
 					 <div class="row-fluid section">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="pull-left"><h4>  ग्राहक खाते तपशील   </h4></div>
								<div class="muted pull-right"> <a href="customer.php" ><i class="icon-arrow-left"></i> मागे पहा  </a></div>
                            </div>
...
error_reporting(0);
	...
	{    	$data_bill[] = $row_bill;}	$bill_total=0; foreach($data_bill as $ecol_bill)	{$total_bill=$ecol_bill['total_amount']; $bill_total=$bill_total + $total_bill;	}
...
$query_payment="select * from all_tran where acc_id = '$cust_id' and acc_type='customer' order by tran_date desc ";
	...
	$data_payment=NULL;
  error_reporting(0);
	while ($row_payment = mysql_fetch_array($result_payment)){ $data_payment[] = $row_payment;	}	$payment_total=0;
	...
	$jtotal=0;
		...
		$jtotal=$jtotal + $total_amount;
	 			    ...
	 			    $p_date= date('d-m-Y', strtotime($pay_date));
 						             ...
 						             echo"<tr>";
 						                  echo"<td> <a href=editPayment.php?id=$pay_id> $count </a> </td>";
										  echo"<td> $p_date </td>";
 										   echo"<td>$total_amount</td>";
